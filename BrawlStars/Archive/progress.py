...
TOTAL_BRAWLERS = 36  # May need updating
...
def update(playertag, player=None):
    ...
    # Update number of coins, star points
    ...
    for b in b_by_tphy:
        ...
        if is_max(brawler):
            ...
            num_input(
                f"{name}: {brawler['points']}/{POINTS[brawler['power']]} -> ",
                brawler, 'points'
            )
...
def prediction(playertag, out_type="boxes", manual=None):
    ...
    total_days = days_for_points + days_for_coins
    today = date.today()
